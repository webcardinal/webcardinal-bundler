...
class Builder {
    ...
    return {
      ...
      buildAll: async () => {
        ...
        if (components.length === 0) {
          throw `No WebCardinal components found! in "${this.devPath}"`;
        }
        ...
        if (packages['@webcardinal/core'] && this.options.targetedBundle !== 'bundle-cardinal') {
      ...
      prerelease: async (components) => {
        let copyConfig = await this.configuration.copyAll();
        copyConfig = copyConfig.filter(config => {
          for (let componnt of components) {
            if (config.name.endsWith(componnt)) {
              return true;
            };
          }
          return false;
  ...
  async merge(components) {
    if (!Array.isArray(components)) {
      components = await this.configuration.components(this.buildPath);
    }
    ...
    await writeFile(path.join(this.buildPath, '../webcardinal.js'), content.js, { recursive: true });
    await writeFile(path.join(this.buildPath, '../webcardinal.css'), content.css, { recursive: true });
  }

  async prerelease(components) {
    await this.configuration.buildAll(components);
    this.runner.run(await this.configuration.prerelease(components))
  }
}
